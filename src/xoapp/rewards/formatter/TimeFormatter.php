...
namespace xoapp\rewards\formatter;
...
class TimeFormatter {
    ...
    public static function isValidFormat(string $format): bool {
        ...
        $string = self::intToString($format);

        return in_array($string, self::VALID_FORMATS);
    }
    ...
    public static function parseTime(string $data): int {
        $int = self::stringToInt($data);
        return self::getFormatTime($int, $data);
    }
}